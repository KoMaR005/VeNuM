...
if (!db.data.chats[m.chat].modohorny && m.isGroup) throw '*[ ⚠️ ] В этой группе отключены команды +18, если вы являетесь администратором этой группы и хотите включить их, введите #вкл nsfw*'
let url = packmen[Math.floor(Math.random() * packmen.length)]
conn.sendButton(m.chat, `_🥵 Pack 3 🥵_`, author, url, [['🔄 СЛЕДУЮЩИЙ 🔄', `/${command}`]], m)
}
handler.help = ['pack3']
handler.tags = ['internet']
handler.command = /^(пакет3)$/i
